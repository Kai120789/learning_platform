...
func New(config *config.Config, logger *zap.Logger, service *MaterialService) *GRPCServer {
	gRPCServer := goGRPC.NewServer(goGRPC.ChainUnaryInterceptor(
		recovery.UnaryServerInterceptor(),
	))

	materialGRPC.RegisterMaterialServer(gRPCServer, NewMaterialsGRPCServer(service, logger))
